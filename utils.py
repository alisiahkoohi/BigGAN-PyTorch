...
def load_weights(G, D, hyper_nets, state_dict, weights_root, experiment_name,
  ...
  if hyper_nets is not None:
    for hyper_net in hyper_nets:
      hyper_net.load_state_dict(
        torch.load('%s/%s.pth' % (root, join_strings('_', [hyper_net.name, name_suffix]))),
        strict=strict)
  # Load state dict
